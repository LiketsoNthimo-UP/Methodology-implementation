...
test_cases = test_cases[test_cases['Service_rate'] == 1]
#test_cases = test_cases[test_cases['Offered_load'] == 64]
test_cases = test_cases[test_cases['Planning_period'] == 2]
...
for i in range(rows):
    ...
    arrival_rate = offered_load * service_rate
    log_base_2_offered_load = math.log(arrival_rate,2)
    days_to_use = math.pow(2,9-log_base_2_offered_load)
    days_to_use_index = int(days_to_use + 1)
    
    if offered_load == 16:
        test_case_results = run_test_case(service_rate, offered_load, planning_period, days_to_use_index)
