...
def update_agent_times(agent_logs, call_features, agent_index):
    ...
    #print('AT: ' + str(arrival_time*60*60) + ', SD: ' + str(call_features.iloc[0,1]*60*60) + ', ST: ' + str(agent_logs.iloc[agent_index,0]*60*60) + ', ET: ' + str(agent_logs.iloc[agent_index,1]*60*60))
...
def update_agent_times(agent_logs, call_features, agent_index):
    agent_logs = agent_logs.copy()
    arrival_time = call_features['Arrival_time']
    agent_logs = update_break_periods(agent_logs, arrival_time)
    
    agent_logs.iloc[agent_index,0] = arrival_time + call_features['Service_duration']
    agent_logs = agent_logs.loc[(agent_logs['Start_time'] < agent_logs['End_time']) & (agent_logs['End_time'] > arrival_time),]
...
def populate_performance_info(shift_schedule_per_day, call_centre_scenario, max_wait):
    ...
    for day in unique_days:
        ...
        waiting_time_list = []
        call_answered_list = []
        for index, call_features in day_scenario.iterrows():
            ...
            if agent_logs.shape[0] > 0:
                ...
                for j in range(agent_rows):
                    ...
                    # Checks that call can be answered before customer runs out of patience and that call arrives before agent shift has ended
                    # also check that the call arrives before the agent leaves for break or that the agent is back before the customer drops
                    if (agent_logs.iloc[j,0] <= call_features['Arrival_time'] + call_features['Time_to_abandon']) and (agent_logs.iloc[j,1] > call_features['Arrival_time']) and ((agent_logs.iloc[j,2] > call_features['Arrival_time']) or (agent_logs.iloc[j,3] <= call_features['Arrival_time'] + call_features['Time_to_abandon'])):
                        waiting_time_list.append(np.maximum(agent_logs.iloc[j,0] - call_features['Arrival_time'],0))
                        call_answered_list.append('yes')
                        agent_logs = update_agent_times(agent_logs, call_features, j)
                        day_answered_calls = day_answered_calls + 1
                        total_answered_calls = total_answered_calls + 1
                        is_unanswered = False
                ...
                if is_unanswered:
                    waiting_time_list.append(call_features['Time_to_abandon'])
                    call_answered_list.append('no')
                    day_dropped_calls = day_dropped_calls + 1
                    total_dropped_calls = total_dropped_calls + 1
        ...
        day_scenario = day_scenario.assign(Waiting_time = waiting_time_list)
        day_scenario = day_scenario.assign(Call_answered = call_answered_list)
        #day_scenario['Waiting_time'] = waiting_time_list
        #day_scenario['Call_answered'] = call_answered_list
        
        call_centre_scenario_operation = day_scenario.copy()
