...
roster_file_path = '../Part 3 datasets/Results_' +  average_time_to_abandon + '.csv'
roster_info = pd.read_csv(roster_file_path)
roster_info = roster_info[['Cycle_number','Start_time','End_time','Break_start','Break_end','Staff_number']]
roster_info = roster_info[roster_info['Staff_number'] > 0]
roster_info.Staff_number = roster_info.Staff_number.astype(int)
roster_info = roster_info.reset_index(drop = True)
...
shift_schedule_per_day = cycle_number_to_shift_day(roster_info, day_range)

performance_info = populate_performance_info(shift_schedule_per_day, call_centre_scenario, max_wait)
...
file_name = 'Performance_info_' + str(average_time_to_abandon) + '.csv'
